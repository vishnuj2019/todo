...
const SampleForm1 = () => {
  ...
  useEffect(() => {
    const suscribtion = watch((value) => {
      console.log("from the useEffect", value);
    });

    return () => suscribtion.unsubscribe();
  }, [watch]);
  //  console.log("outside", watch());
  const {
    append,
    remove,
    fields: users_Array_Fields,
  ...
  useEffect(() => {
    ...
    if (typeof image !== "string" && image instanceof FileList) {
      let file = Array.from(image);
      if (file.length) setPreviewImage(URL.createObjectURL(file[0] as File));
    } else {
  ...
  return (
    <div className="w-1/3">
      <h1>Form with Child Input</h1>
      <h1>{count}</h1>
      <form
        noValidate
        onSubmit={handleSubmit(onSubmit)}
        className="flex flex-col gap-y-2 mt-7"
      >
        <FormInput
          label="Username"
        ...
        <FormInput
          label="Channal"
          register={register("channal", { required: "Channal is required" })}
          type="text"
          message={errors.channal?.message!}
        />
        <FormInput
          label="City"
        ...
        <FormInput
          ...
          register={register("phoneNumbers.0", {
            required: "Primary Number is required",
          })}
          type="number"
          message={errors.phoneNumbers?.[0]?.message!}
        />
        <FormInput
          label="Personal Number"
          register={register("phoneNumbers.1", {
            required: "Personal Number is required",
          })}
          type="number"
          message={errors.phoneNumbers?.[1]?.message!}
        />
        {users_Array_Fields?.map((field, index) => (
          <div>
            <FormInput
              key={field.id}
              label="name"
              register={register(`users.${index}.name`, {
                required: "Name is required",
              })}
              type="text"
              message={errors.users?.[index]?.name?.message!}
            />
            <FormInput
              key={index}
              label="age"
              register={register(`users.${index}.age`, {
                required: "Age is required",
              })}
              type="number"
              message={errors.users?.[index]?.age?.message!}
            />
            {index > 0 && (
        ...
        <img className="w-80 h-80" src={previewImage} />
